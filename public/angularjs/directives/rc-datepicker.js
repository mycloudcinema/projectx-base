...
(function (ng) {
	...
	function rcDatepicker(rcDateHandler, $locale) {
		...
		return {
			...
			restrict: "E",
			template: '<div class="text-container input-group"><input type="text" class="" placeholder="{{placeholder}}"/><span class="input-group-addon" ng-click="iconClicked()""><i class="fa fa-calendar"></i></span><div class="focus"><div></div></div></div>',
			// template: '<div class="text-container"><div style="position: absolute;right: 24px;top: 11px;"><i class="fa fa-calendar" ng-click="iconClicked()"></i></div><input type="text" class="" placeholder="{{placeholder}}"/><div class="focus"><div></div></div></div>',
			scope: {
				minDate: "=",
				maxDate: "=",
				viewMode: "=",
			...
			link: function (scope, element, attrs, ctrl) {
				...
				if (scope.options && scope.options.locale) {
					datepickerLocale = moment.locale(scope.options.locale)
				} else {
					datepickerLocale = moment.locale($locale.MOMENT.locale)
				}

				// Override the starting day of the week with the one supplied by
				// the calling code, or default to Thursday.
				try {
					var datePickerDOW = (scope.options && scope.options.startingDay) ? scope.options.startingDay : 4;
