/**
 *  Copied from
 *  https://github.com/bentorfs/angular-bootstrap-multiselect/blob/master/dist/angular-bootstrap-multiselect.js
...
(function (ng) {
	...
	ng.module("rc.directives")
		.directive('rcMultiSelect', ['$filter', '$document', '$log', function ($filter, $document, $log) {
		return {
			restrict: 'AE',
			scope: {
				options: '=',
			...
			"                <i class='fa fa-check'></i> {{labels.selectAll || 'Select All'}}\n" +
			"            </a>\n" +
			"        </li>\n" +
			"        <li ng-show='showUnselectAll'>\n" +
			"            <a ng-click='unselectAll()' href=''>\n" +
			"                <i class='fa fa-times'></i> {{labels.unselectAll || 'Unselect All'}}\n" +
			"            </a>\n" +
			"        </li>\n" +
			...
			"					ng-click='toggleItem(option); $event.stopPropagation()' style='overflow-x: hidden; text-overflow: ellipsis'>\n" +
			"                <i class='fa fa-times'></i> {{getDisplay(option)}}\n" +
			"            </a>\n" +
			"        </li>\n" +
			"        <li ng-show='selectedOptions.length > 0' class='divider'></li>\n" +
			"        <li ng-show='showSearch'>\n" +
			"            <div class='dropdown-header'>\n" +
			"                <input type='text' class='form-control input-sm' style='width: 100%;' ng-model='searchFilter' " +
			...
			link: function ($scope, $element, $attrs, $ngModelCtrl) {
				...
				// Added this because options were loaded after directive finished initializing (in my case)
				...
				$scope.getRecursiveProperty = function (object, path) {
				...
				$scope.getId = function (item) {
					if (angular.isString(item)) {
						...
						if ($scope.idProp) {
							return $scope.getRecursiveProperty(item, $scope.idProp);
						} else {
				...
				$scope.getDisplay = function (item) {
					if (angular.isString(item)) {
						...
						if ($scope.displayProp) {
							return $scope.getRecursiveProperty(item, $scope.displayProp);
						} else {
