...
angular.module('mgcrea.ngStrap.alert', ['mgcrea.ngStrap.modal'])
  ...
  .provider('$alert', function() {
    ...
    this.$get = ["$uibModal", "$timeout", function($uibModal, $timeout) {
...
angular.module('mgcrea.ngStrap.modal', ['mgcrea.ngStrap.helpers.dimensions'])
  ...
  .provider('$uibModal', function() {
    ...
    this.$get = ["$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "$timeout", "$sce", "dimensions", function($window, $rootScope, $compile, $q, $templateCache, $http, $animate, $timeout, $sce, dimensions) {
      ...
      function ModalFactory(config) {
        ...
        // Common vars
        var options = $uibModal.$options = angular.extend({}, defaults, config);
        $uibModal.$promise = fetchTemplate(options.template);
        var scope = $uibModal.$scope = options.scope && options.scope.$new() || $rootScope.$new();
        ...
        $uibModal.$id = options.id || options.element && options.element.attr('id') || '';
        ...
        $uibModal.$isShown = scope.$isShown = false;

        // Support contentTemplate option
        if(options.contentTemplate) {
          $uibModal.$promise = $uibModal.$promise.then(function(template) {
        ...
        $uibModal.$promise.then(function(template) {
        ...
        $uibModal.show = function() {
          if($uibModal.$isShown) return;
          ...
          modalElement = $uibModal.$element = modalLinker(scope, function(clonedElement, scope) {});

          if(scope.$emit(options.prefixEvent + '.show.before', $uibModal).defaultPrevented) {
            return;
          }
          ...
          $uibModal.$isShown = scope.$isShown = true;
          safeDigest(scope);
          ...
          if(options.keyboard) {
            modalElement.on('keyup', $uibModal.$onKeyUp);
          }
        };

        function enterAnimateCallback() {
          scope.$emit(options.prefixEvent + '.show', $uibModal);
        }

        $uibModal.hide = function() {
          if(!$uibModal.$isShown) return;

          if(scope.$emit(options.prefixEvent + '.hide.before', $uibModal).defaultPrevented) {
            return;
          }
          ...
          if(options.keyboard) {
            modalElement.off('keyup', $uibModal.$onKeyUp);
          }
        };

        function leaveAnimateCallback() {
          scope.$emit(options.prefixEvent + '.hide', $uibModal);
        ...
        $uibModal.toggle = function() {

          $uibModal.$isShown ? $uibModal.hide() : $uibModal.show();

        };

        $uibModal.focus = function() {
          modalElement[0].focus();
        };

        // Protected methods

        $uibModal.$onKeyUp = function(evt) {

          if (evt.which === 27 && $uibModal.$isShown) {
            $uibModal.hide();
            evt.stopPropagation();
          }
        ...
        function hideOnBackdropClick(evt) {
          ...
          options.backdrop === 'static' ? $uibModal.focus() : $uibModal.hide();
        }
  ...
  .directive('bsModal', ["$window", "$sce", "$uibModal", function($window, $sce, $uibModal) {
